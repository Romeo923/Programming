...
namespace FaceRecogPCA
{
	...
	public class Matrix : IMatrix
	{
		...
		{
			if ((rows != B.Rows) || (columns != B.Columns)) throw new ArgumentException("Matrix dimension do not match.");
			Matrix X = new Matrix(rows, columns);
			double[][] x = X.Array;
			for (int i = 0; i < rows; i++)
				for (int j = 0; j < columns; j++)
		...
		{
			if (B.Rows != this.columns) throw new ArgumentException("Matrix dimensions are not valid.");
		...
		*/
		...
		class CholeskyDecomposition : ICholeskyDecomposition
		{
			Matrix L;
			bool isSymmetric;
			...
			{
				if (!A.IsSquare) throw new ArgumentNullException("Matrix is not square.");
			...
			public IMatrix Solve(IMatrix rhs)
			{
				if (rhs.Rows != L.Rows) throw new ArgumentException("Matrix dimensions do not match.");
				if (!isSymmetric) throw new InvalidOperationException("Matrix is not symmetric.");
				if (!isPositiveDefinite) throw new InvalidOperationException("Matrix is not positive definite.");
		...
		class LuDecomposition : ILuDecomposition
		{
			Matrix LU;
			int pivotSign;
			int[] pivotVector;
		...
		class QrDecomposition : IQrDecomposition
		{
			Matrix QR;
			double[] Rdiag;
		...
		class SingularValueDecomposition : ISingularValueDecomposition
		{
			Matrix U;
			Matrix V;
		...
		class EigenvalueDecomposition : IEigenvalueDecomposition
		{
			int n;           	// matrix dimension
			double[] d, e; 		// storage of eigenvalues.
			Matrix V;    			// storage of eigenvectors.
			Matrix H;    			// storage of nonsymmetric Hessenberg form.
			double[] ort;    	// storage for nonsymmetric algorithm.
			double cdivr, cdivi;
			bool isSymmetric;
