...
class Network(object):
    ...
    def __init__(self,X,Y,numLayers,batchsize,dropOut =
        1.0,activationF=ActivationType.SIGMOID, lastLayerAF= ActivationType.SIGMOID):
        self.X = X
        self.Y = Y
    ...
    def Train(self, epochs,learningRate, lambda1, batchsize=1,
        LROptimization=LROptimizationType.NONE,doBatchNorm=False):
        itnum = 0
    ...
    def UpdateGradsBiases(self, learningRate, lambda1, batchSize, LROptimization, itnum, doBatchNorm):
        ...
        for ln in range(len(self.numLayers)):
            if (LROptimization == LROptimizationType.NONE):
            ...
            elif (LROptimization == LROptimizationType.ADAM):
