...
LinkedList* page_number_refrence = NULL;
int fault_count = 0;
...
//This function Search and finds the frame index in the page_table
//that is the furthest away in the page reference list
//The following functions are available to call:
//page_number_refrence->Search
int swapPage_Optimum(int page_to_add)
{
	int local_frame_index = 0;
	unsigned int tmp_index, save_index = 0;
	//TODO> Add your logic here>
	
	for (tmp_index = 1; tmp_index < NUMBER_OF_FRAMES; tmp_index++)
	{
		//compares length of current index to previous and saves the larger one
		save_index = page_number_refrence->Search(page_table[tmp_index]) > page_number_refrence->Search(page_table[tmp_index - 1]) ? tmp_index : save_index;
	}
...
OPCODE fetch()
{
	...
	if (opcode == MOV) {
		...
		//The following functions are available to call:
		//is_PageLoaded, swapPage_Optimum, page_number_refrence->Remove, 
		//findEmptyFrame, update_page_table, load_LogicalToPhysical

		if (!is_PageLoaded(page, &frame_index)) {  //page not in physical memory
			...
			if (frame_index == NOT_FOUND) {   
				...
				frame_index = swapPage_Optimum(page);
		...
		page_number_refrence->Remove(page);
...
{
	OPCODE opcode;
	page_number_refrence = new LinkedList();
	...
	std::cout << "\n\nOptimum: # of Page Fault: " << fault_count << "\n";

	delete page_number_refrence;
	return 0;
}
	...
	if (vm_file_Ptr != NULL)
	{
		for (j = 0; j < length; j++)
		{
			...
			if (ret > 0)
			{
				...
				{
					std::cout << page << "  ";
					page_number_refrence->AddToTail(page, 0);
				}
				count++;
			}
			else 
