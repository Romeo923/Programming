...
class Disk {
private:
	HANDLE	FileDescriptor; // File descriptor of disk image
	...
	void sanity_check(int blocknum, unsigned char *data);
...
public:
	...
	// Destructor
	~Disk();
	size_t get_Blocks() {	return Blocks;	}
	void set_Blocks(size_t size) { Blocks = size; }
	// Open disk image
	...
	int read(int blocknum, unsigned char *data);

	// Write block to disk
	// @param	blocknum    Block to write to
	// @param	data	    Buffer to write from
	int write(int blocknum, unsigned char *data);
};
